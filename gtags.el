...
;; highlight
(defun gtags-select-setup-buffer ()
  (goto-char (point-min))
  (let (prev-file
        file
        ln
        (extract-until-space
         (lambda ()
           (delete-and-extract-region
            (point) (progn (forward-word 1)
                           (1- (search-forward " " nil t 1)))))))
    (while (not (eobp))
      (funcall extract-until-space)
      (setq ln (string-trim (funcall extract-until-space)))
      (setq file (string-trim (funcall extract-until-space)))
      ;; (insert
      ;;  (replace-regexp-in-string
      ;;   "01;31m" "01;46m" (buffer-substring (point) (line-end-position))))

      (goto-char (line-beginning-position))
      (unless (and prev-file (equal prev-file file))
        (insert (if prev-file "\n" "")
                "[01;32m" file "[m" "\n"))
  ...
  (save-excursion
    (let (line file)
      ...
      (if (not (looking-at "[0-9]+"))
        ...
        (while (and (not (bobp)) (looking-at "[0-9]+"))
          (forward-line -1))
...
;(defface gtags-select-line-number-face `((t (:foreground "gold"))) "" :group 'faces)
(defface gtags-select-class-face
  `((t (:inherit font-lock-keyword-face :background "tomato")))
  ""
  :group 'faces)

(defvar gtags-select-keywords
  `((" \\(class\\|struct\\|enum\\)"
     (1 'gtags-select-class-face nil t))
    ("^\\([0-9]+\\):"
     (1 'shadow nil t))
    ))
...
`gtags-select-mode-hook' with no args, if that value is non-nil."
    ...
    (font-lock-add-keywords nil gtags-select-keywords)
...
(define-derived-mode gtags-select-mode-c++ c++-mode "Gtags-Select"
    (gtags-select-mode))
